...
# Запускаємо команду для фреймворку Django.
# Вона збирає всі статичні файли (картинки, CSS, JS) в одну папку для зручної роздачі сервером.
# --noinput означає, що скрипт не буде запитувати підтвердження "Yes/No" у консолі.
...
ENTRYPOINT ["gunicorn", "gamehub.wsgi:application", "-b", "0.0.0.0:8000"]
